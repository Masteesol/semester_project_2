...
export  function createElement (type, classNameInput = null, attributeTypeInput = null, attributeValueInput = null, textValue = null, htmlTemplate = null) {
    const container = document.createElement(type);

    const [className, attributeType, attributeValue] = [classNameInput, attributeTypeInput, attributeValueInput].map(arg => arrayFromString(arg))
    if(className) {
...
export function modifyClassNames(elementInput, addClassNamesInput = null, removeClassNamesInput = null, swap = null) {
    const [ addClassNames, removeClassNames] = [addClassNamesInput, removeClassNamesInput].map(arg => arrayFromString(arg))
...
export function addListener(elementInput, functionName, eventType = "click") {
    ...
    if(element && functionName) {
        if(Array.isArray(element) && !Array.isArray(functionName)) {
            ...
            element.forEach((el, index) => {
                if(Array.isArray(el)) {
                    ...
                    el.forEach(item => item.addEventListener(eventType, functionName[index]))
                } else {
        ...
        console.log("addListener: Error: null value entered: ", element, "and", functionName)
    }
}
