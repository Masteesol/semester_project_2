...
export async function openProductOverview(category) {
    ...
    selectElement("#body-interface").insertAdjacentElement("beforebegin", productOverview(data, category));
    addListener(
...
export function productOverview (products, category) {
    ...
    const container = createElement("div", ["container-fluid", "position-absolute", "flow-y-scroll", "increase-width"], ["style", "id"], ["z-index: 6000; height: 100vh; right: 0; padding-top: 5rem", "products-grid"])
    
    const wrapper = createElement("div", ["wrapper", "mx-auto", "mb-5"], "style", "max-width: 1000px");
    ...
    products.forEach(product => {
        ...
        if(tag === "base") {
            ...
            const row = createElement("div", ["card-row", "row", "mt-4", "m-screen-flex-column"]);
            let count = 0;
            products.forEach(item => {
                const {title : titleInner, tag : tagInner, category: categoryInner } = item.attributes;
