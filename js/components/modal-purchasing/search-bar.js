...
export default async function () {
  try {
    ...
    const container = createElement(
      "div",
      "container-fluid w-100 position-absolute bg-light",
      "style | id",
      "top: 0; z-index: 7000; min-height: 100vh | product-list"
    );
      ...
      addListener(
        ...
        () => {
          ...
          selectElement("#all-cards").childNodes.forEach((element) => {
            ...
            if (
              !elementData.includes(modifiedSearch) ||
              !title.includes(modifiedSearch)
            ) {
...
function createCard(data) {
  const { title, image_url, price, tag, category, description } =
    data.attributes;
  ...
  return createElement(
    "div",
    "col-3 text-primary bg-white mx-auto shadow rounded my-5 d-flex flex-column justify-content-between",
    "style | id |  data",
    `width: 15rem; height: 20rem | card-${titleModified} | ${descriptionModified}`,
    "",
    template
  );
}
